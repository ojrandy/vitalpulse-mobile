...
## 11. Client-side route separation (UX/defense-in-depth layer — not the authorization boundary)

Donor, hospital, and admin are treated as three separate route subtrees in the mobile app, so one role's screens (and whatever mock/live data they render) can never mount inside another role's session:

- The root layout (`app/_layout.tsx`) wraps `(donor)`, `(hospital)`, and `(admin)` — each an Expo Router group containing that role's entire screen tree, tabs and pushed detail screens alike — in `<Stack.Protected guard={...}>`, keyed off `role` from `useAuthStore`. A group whose guard is false is not merely hidden; it is unmounted from the navigator, so a deep link, a typed URL (web), or a stale `router.push` to another role's screen (e.g. `(admin)/admin-request-detail/[id]` or `(hospital)/inventory-detail/[bloodType]`) cannot resolve while signed in as a different role.
- Every donor/hospital/admin screen — including detail screens like `donation-history`, `admin-request-detail`, `inventory-detail` — lives inside its role's group folder. A screen placed loose at the `app/` root (a sibling of the groups, not nested in one) is **not covered by the guard**, regardless of what the guard checks — this was the original bug this section closes.
- This reuses the same `role` field already documented in §3 as cosmetic/routing-only, and the same caveat applies here: `Stack.Protected` stops the *client* from rendering another role's UI, it is not proof against a malicious client. The actual authorization boundary is, and remains, Firestore Security Rules + Cloud Functions checking custom claims (§3–§5) once real Firebase Auth (custom claims, `hasRole()`, per-hospital scoping) replaces the current auth stub. Do not treat route separation as a substitute for a rules-test-suite case.
- `(auth)` and the root `index` screen are intentionally left unguarded: they hold no role-specific data, and the donor onboarding flow (notification primer, biometric setup, profile setup) runs under `(auth)` after `setSignedIn()` is called but before the donor is routed into `(donor)`, so it must stay reachable while `role` is already non-null.
